...
```{r}
trainStats <- subset(train,train$stationName==rownames(df))
...
ClusterTrainData <- function(df) {
  trainStats <- aggregate(cbind(alightCount_mean, alightCount_count, boardCount_mean, boardCount_count, waitTime_mean,trainCount_mean) ~ hour+stationName+direction+line,data=train,FUN=mean)
  Temp2 <- data.frame()
  for (i in 1:length(rownames(df))) {
